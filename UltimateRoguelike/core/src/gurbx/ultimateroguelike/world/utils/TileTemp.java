...
public class TileTemp {
	...
	public static boolean isAccessible(int x, int y, int fromX, int fromY, String[][] tiles) {
		try {
			if (tiles[x][y].equals(DungeonConstants.EMPTY) == false) return false;
			
			//If it is the first tile that is being placed
			if (tiles[fromX][fromY].equals(DungeonConstants.GROUND) == false) {
				if (tiles[x+1][y].equals(DungeonConstants.EMPTY) == false) return false;
				if (tiles[x+1][y+1].equals(DungeonConstants.EMPTY) == false) return false;
				if (tiles[x][y+1].equals(DungeonConstants.EMPTY) == false) return false;
				if (tiles[x-1][y].equals(DungeonConstants.EMPTY) == false) return false;
				if (tiles[x-1][y-1].equals(DungeonConstants.EMPTY) == false) return false;
				if (tiles[x][y-1].equals(DungeonConstants.EMPTY) == false) return false;
				if (tiles[x+1][y-1].equals(DungeonConstants.EMPTY) == false) return false;
...
//			if (isTileEmpty(x-1, y+1, fromX, fromY, tiles) == false) return false;
	...
	public static boolean isDeadEnd(int x, int y, String[][] tiles) {
		if (tiles[x][y].equals(DungeonConstants.GROUND) == false) return false; // can't be dead end if not ground
		
		int connections = 0;
		try { if (tiles[x+1][y].equals(DungeonConstants.EMPTY) == false) connections++;
		} catch (IndexOutOfBoundsException e) {}
		try { if (tiles[x-1][y].equals(DungeonConstants.EMPTY) == false) connections++;
		} catch (IndexOutOfBoundsException e) {}
		try { if (tiles[x][y+1].equals(DungeonConstants.EMPTY) == false) connections++;
		} catch (IndexOutOfBoundsException e) {}
		try { if (tiles[x][y-1].equals(DungeonConstants.EMPTY) == false) connections++;
		} catch (IndexOutOfBoundsException e) {}
